...
export const SelectFund = () => {
    ...
    async function fetchData() {
      if (uid) {
        ...
        const userData = await axios.get(
          `http://localhost:8000/db/userdata=${uid}`
        );
        let sumOfInvest = 0;
  ...
  function saveTaxGoal(e) {
    ...
    if (Funds.includes("")) {
      ...
      axios
        .post("http://localhost:8000/db/save_tax_goal", {
          Name: "ลดหย่อนภาษี",
          Funds: { ...Funds },
          userId: uid,
            ...
            >
              ...
              >
                <Typography
                  ...
                  marginBottom={1}
                >
                  เพื่อลดหย่อนภาษีให้ได้สูงสุด ต้องซื้อกองทุนเพิ่ม{" "}
                  {netIncome - 150000} บาท
                </Typography>
                <Typography>
                  คุณซื้อกองทุน RMF ได้สูงสุด :{" "}
                  {Math.min(500000, beforeReduction * 0.3)} บาท
                </Typography>
                <Typography component={"span"}>
                  คุณซื้อกองทุน SSF ได้สูงสุด :{" "}
                  {Math.min(200000, beforeReduction * 0.3)} บาท
                </Typography>
              </div>
              <div
                style={{
