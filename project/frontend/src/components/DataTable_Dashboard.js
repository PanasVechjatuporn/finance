...
export default function MonthDataTable() {
  ...
  const handleNewMonthClick = () => {
    openNewMonthModal ? setOpenNewMonthModal(false) : setOpenNewMonthModal(true)
  };
  ...
  const handleAddNewYear = async () => {
    ...
    await Promise.all([
      ...
      setUserData(prevUserData => [...prevUserData, { year: newYear.toString(), data: [] }])
    ])
    .then(() => {
  ...
  return (
    <Container>
              ...
              <TableBody>
                ...
                {currentYearData.data.length !== 12 ? (
                  <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
                    <TableCell colSpan={6} align="center" onClick={(e) => {
                      handleNewMonthClick()
                    }}
                      style={{ cursor: 'pointer' }}
                    >
                      <IconButton children={<AddCircleOutlineIcon></AddCircleOutlineIcon>
                      }></IconButton>
                    </TableCell>
                  </TableRow>) : (<></>)}
              </TableBody>
          ...
          <Box style={{ position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "orange", display: "flex", justifyContent: "flex-end" }}>
            <FormControl variant="standard" style={{ marginLeft: "10px", marginRight: "12px" }}>
              ...
              >
                ...
                <MenuItem style={{ display: 'flex', justifyContent: 'center' }} onClick={(e) => {
                  handleAddNewYear()
                }}>
      ...
      <EditMonthDataModal show={openNewMonthModal} onClose={handleNewMonthClick} mode="newmonth"></EditMonthDataModal>
    </Container>
  );
}